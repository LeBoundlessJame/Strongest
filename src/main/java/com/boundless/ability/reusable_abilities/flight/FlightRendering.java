...
public class FlightRendering {
    ...
    public static HashMap<AbstractClientPlayerEntity, Float> playerTicksMap = new HashMap<>();
    ...
    public static void renderFlight(AbstractClientPlayerEntity player, MatrixStack matrixStack, float f, float g, float tickDelta, float i, PlayerEntityRenderer renderer) {
        ...
        float playerTime = playerTicksMap.getOrDefault(player, (float) player.age);
        playerTime += 1.0f;
        playerTicksMap.put(player, playerTime);

        if (player.isSprinting()) {
            matrixStack.multiply(RotationAxis.POSITIVE_Y.rotation((float) Math.sin((playerTime + tickDelta) * 0.025f) / 5f));
        } else {
            matrixStack.translate(0, (float) Math.sin((playerTime + tickDelta) * 0.025f) / 5f, 0);
        }
    }
}
