...
public class VFXUtils {
    ...
    public static ParticleEmitterInfo createEffectInstance(Entity user, String effectName, Vec3d pos, Float scale, boolean bindToEntity) {
        ParticleEmitterInfo particleEmitter = ParticleEmitterInfo.create(user.getWorld(), BoundlessAPI.id(effectName), BoundlessAPI.id(effectName + user.getId()));
    ...
    public static void destroyEffectInstance(String effectName, int id) {
        var effect = EffectRegistry.get(BoundlessAPI.id(effectName));
        if (effect == null) return;
        Optional<ParticleEmitter> emitter = effect.getNamedEmitter(ParticleEmitter.Type.WORLD, BoundlessAPI.id(effectName + id));
