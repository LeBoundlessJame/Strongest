...
public class GamaEntityRenderer<T extends GamaEntity> extends AzEntityRenderer<T> {
    ...
    public GamaEntityRenderer(EntityRendererFactory.Context context) {
        super(AzEntityRendererConfig.<T>builder(GEO, TEXTURE).setAnimatorProvider(GamaAnimator::new).setPrerenderEntry(pipelineContext -> {
            T gama = pipelineContext.animatable();

            boolean isTheBoss = gama.hasCustomName() && gama.getCustomName().equals(Text.literal("Large employer"));

            pipelineContext.bakedModel().getBone("big_boss").ifPresent(bone -> {
                bone.setHidden(!isTheBoss);
            });

            pipelineContext.bakedModel().getBone("big_boss2").ifPresent(bone -> {
                bone.setHidden(!isTheBoss);
            });
    ...
    @Override
    public void render(@NotNull T gama, float entityYaw, float partialTick, @NotNull MatrixStack poseStack, @NotNull VertexConsumerProvider bufferSource, int packedLight) {
